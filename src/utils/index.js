...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://exchange.ksfswap.finance/#` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521' ? 'KCS' : token0Address}/${'KCS'}`
    )
  } else {
    return (
      `https://exchange.ksfswap.finance/#` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521' ? 'KCS' : token0Address}/${token1Address === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521' ? 'KCS' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://exchange.ksfswap.finance/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://exchange.ksfswap.finance/#/swap?inputCurrency=${token0Address === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521' ? 'KCS' : token0Address
      }&outputCurrency=${token1Address === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521' ? 'KCS' : token1Address}`
  }
}
...
export async function getShareValueOverTime(pairAddress, timestamps) {
  ...
  for (var row in result?.data) {
    ...
    if (timestamp) {
      values.push({
        ...
        token0DerivedKCS: result.data[row].token0.derivedKCS,
        token1DerivedKCS: result.data[row].token1.derivedKCS,
  ...
  for (var brow in result?.data) {
    ...
    if (timestamp) {
      values[index].kcsPrice = result.data[brow].kcsPrice
      values[index].token0PriceUSD = result.data[brow].kcsPrice * values[index].token0DerivedKCS
      values[index].token1PriceUSD = result.data[brow].kcsPrice * values[index].token1DerivedKCS
      index += 1
    }
  }
...
export const urls = {
  showTransaction: (tx) => `https://explorer.kcc.io/en/tx/${tx}/`,
  showAddress: (address) => `https://explorer.kcc.io/en/address/${address}/`,
  showToken: (address) => `https://explorer.kcc.io/en/token/${address}/`,
  showBlock: (block) => `https://explorer.kcc.io/en/block/${block}/`,
}
