...
function formatPricesForEarlyTimestamps(position): Position {
  if (position.timestamp < PRICE_DISCOVERY_START_TIMESTAMP) {
    ...
    // WKCS price
    if (position.pair?.token0.id === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521') {
      position.token0PriceUSD = 203
    }
    if (position.pair?.token1.id === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521') {
 ...
 * @param currentKCSPrice // current price of kcs used for usd conversions
 */
export async function getHistoricalPairReturns(startDateTimestamp, currentPairData, pairSnapshots, currentKCSPrice) {
  ...
  for (const index in dayTimestamps) {
    ...
    if (!positionT1) {
      positionT1 = {
        ...
        token0PriceUSD: currentPairData.token0.derivedKCS * currentKCSPrice,
        token1PriceUSD: currentPairData.token1.derivedKCS * currentKCSPrice,
      }
    }
 ...
 * @param kcsPrice
 */
export async function getLPReturnsOnPair(user: string, pair, kcsPrice: number, snapshots) {
  // initialize values
  ...
  const currentPosition: Position = {
    ...
    token0PriceUSD: pair.token0.derivedKCS * kcsPrice,
    token1PriceUSD: pair.token1.derivedKCS * kcsPrice,
  }
