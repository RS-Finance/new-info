...
async function getBulkPairData(pairList, kcsPrice) {
    ...
    let pairData = await Promise.all(
      current &&
        current.data.pairs.map(async (pair) => {
          ...
          data = parseData(data, oneDayHistory, twoDayHistory, oneWeekHistory, kcsPrice, b1)
          return data
        })
    )
    return pairData
...
function parseData(data, oneDayData, twoDayData, oneWeekData, kcsPrice, oneDayBlock) {
  // get volume changes
  ...
  data.trackedReserveUSD = data.trackedReserveKCS * kcsPrice
  ...
  if (data?.token0?.id === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521') {
    data.token0.name = 'kcs (Wrapped)'
    data.token0.symbol = 'KCS'
  }
  if (data?.token1?.id === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521') {
    data.token1.name = 'kcs (Wrapped)'
    data.token1.symbol = 'KCS'
  }
...
export function Updater() {
  ...
  useEffect(() => {
    async function getData() {
      ...
      let topPairs = await getBulkPairData(formattedPairs, kcsPrice)
      topPairs && updateTopPairs(topPairs)
    }
    kcsPrice && getData()
  }, [kcsPrice, updateTopPairs])
  return null
}
...
export function useDataForList(pairList) {
  ...
  useEffect(() => {
    ...
    if (kcsPrice && pairList && pairList.length > 0 && !fetched && !stale) {
      setStale(true)
      fetchNewPairData()
    }
  }, [kcsPrice, state, pairList, stale, fetched])
  ...
  useEffect(() => {
    async function fetchData() {
      if (!pairData && pairAddress) {
        let data = await getBulkPairData([pairAddress], kcsPrice)
        data && update(pairAddress, data[0])
      }
    }
    if (!pairData && pairAddress && kcsPrice && isAddress(pairAddress)) {
      fetchData()
    }
  }, [pairAddress, pairData, update, kcsPrice])
