...
export function useUserPositionChart(position, account) {
  ...
  const [currentKCSPrice] = useKcsPrice()
...
export function useUserPositions(account) {
  ...
  useEffect(() => {
        ...
        if (result?.data?.liquidityPositions) {
          let formattedPositions = await Promise.all(
            result?.data?.liquidityPositions.map(async (positionData) => {
              const returnData = await getLPReturnsOnPair(account, positionData.pair, kcsPrice, snapshots)
              return {
                ...positionData,
                ...returnData,
              }
    ...
    if (!positions && account && kcsPrice && snapshots) {
      fetchData(account)
    }
  }, [account, positions, updatePositions, kcsPrice, snapshots])
