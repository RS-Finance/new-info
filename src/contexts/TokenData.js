...
import { useKcsPrice } from './GlobalData'
...
const getTopTokens = async (kcsPrice, kcsPriceOld) => {
    ...
    let bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async (token) => {
          ...
          const currentLiquidityUSD = data?.totalLiquidity * kcsPrice * data?.derivedKCS
          const oldLiquidityUSD = oneDayHistory?.totalLiquidity * kcsPriceOld * oneDayHistory?.derivedKCS

          // percent changes
          const priceChangeUSD = getPercentChange(
            data?.derivedKCS * kcsPrice,
            oneDayHistory?.derivedKCS ? oneDayHistory?.derivedKCS * kcsPriceOld : 0
          )

          // set data
          data.priceUSD = data?.derivedKCS * kcsPrice
          ...
          if (!oneDayHistory && data) {
            ...
            data.oneDayVolumeKCS = data.tradeVolume * data.derivedKCS
            data.oneDayTxns = data.txCount
          }

          if (data.id === '0x4446Fc4eb47f2f6586f9fAAb68B3498F86C07521') {
            data.name = 'Kcser (Wrapped)'
            data.symbol = 'KCS'
          }
...
const getTokenData = async (address, kcsPrice, kcsPriceOld) => {
  const utcCurrentTime = dayjs()
  ...
  try {
    ...
    const priceChangeUSD = getPercentChange(
      data?.derivedKCS * kcsPrice,
      parseFloat(oneDayData?.derivedKCS ?? 0) * kcsPriceOld
    )

    const currentLiquidityUSD = data?.totalLiquidity * kcsPrice * data?.derivedKCS
    const oldLiquidityUSD = oneDayData?.totalLiquidity * kcsPriceOld * oneDayData?.derivedKCS
...
const getIntervalTokenData = async (tokenAddress, startTime, interval = 3600, latestBlock) => {
  ...
  try {
    ...
    // format token KCS price results
    let values = []
    for (var row in result) {
      let timestamp = row.split('t')[1]
      let derivedKCS = parseFloat(result[row]?.derivedKCS)
      if (timestamp) {
        values.push({
          timestamp,
          derivedKCS,
    ...
    // go through kcs usd prices and assign to original values array
    let index = 0
    for (var brow in result) {
      let timestamp = brow.split('b')[1]
      if (timestamp) {
        values[index].priceUSD = result[brow].kcsPrice * values[index].derivedKCS
        index += 1
      }
    }
...
export function Updater() {
  ...
  const [kcsPrice, kcsPriceOld] = useKcsPrice()
  useEffect(() => {
    async function getData() {
      // get top pairs for overview list
      let topTokens = await getTopTokens(kcsPrice, kcsPriceOld)
      topTokens && updateTopTokens(topTokens)
    }
    kcsPrice && kcsPriceOld && getData()
  }, [kcsPrice, kcsPriceOld, updateTopTokens])
  return null
}
...
export function useTokenData(tokenAddress) {
  ...
  useEffect(() => {
    if (!tokenData && kcsPrice && kcsPriceOld && isAddress(tokenAddress)) {
      getTokenData(tokenAddress, kcsPrice, kcsPriceOld).then((data) => {
        update(tokenAddress, data)
      })
    }
  }, [kcsPrice, kcsPriceOld, tokenAddress, tokenData, update])
