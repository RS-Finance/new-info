...
import { useKcsPrice } from '../contexts/GlobalData'
...
function PairPage({ pairAddress, history }) {
  ...
  const token0USD =
    token0?.derivedKCS && kcsPrice ? formattedNum(parseFloat(token0.derivedKCS) * parseFloat(kcsPrice), true) : ''

  const token1USD =
    token1?.derivedKCS && kcsPrice ? formattedNum(parseFloat(token1.derivedKCS) * parseFloat(kcsPrice), true) : ''
                  ...
                  <TYPE.main fontSize={'16px'} lineHeight={1} fontWeight={500} ml={'4px'}>
                    {token0 && token1
                      ? `1 ${formattedSymbol0} = ${token0Rate} ${formattedSymbol1} ${
                          parseFloat(token0?.derivedKCS) ? '(' + token0USD + ')' : ''
                        }`
                      : '-'}
                  </TYPE.main>
                </RowFixed>
                  ...
                  <TYPE.main fontSize={'16px'} lineHeight={1} fontWeight={500} ml={'4px'}>
                    {token0 && token1
                      ? `1 ${formattedSymbol1} = ${token1Rate} ${formattedSymbol0}  ${
                          parseFloat(token1?.derivedKCS) ? '(' + token1USD + ')' : ''
                        }`
                      : '-'}
                  </TYPE.main>
                </RowFixed>
              ...
              >
                <TokenDetailsLayout>
                  ...
                  <ButtonLight color={backgroundColor}>
                    <Link color={backgroundColor} external href={'https://explorer.kcc.io/en/address/' + pairAddress}>
                      View on Explorer ↗
                    </Link>
                  </ButtonLight>
                </TokenDetailsLayout>
