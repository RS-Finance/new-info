      ...
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            ...
            <Link lineHeight={'145.23%'} href={'https://explorer.kcc.io/en/address/' + account} target="_blank">
              {' '}
              {account?.slice(0, 42)}{' '}
            </Link>
          </TYPE.body>
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WKCS') {
                        p.pair.token1.symbol = 'KCS'
                      }
                      if (p.pair.token0.symbol === 'WKCS') {
                        p.pair.token0.symbol = 'KCS'
                      }
                      return (
