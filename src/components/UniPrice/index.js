...
export default function UniPrice() {
  ...
  const daiPerKcs = daiPair ? parseFloat(daiPair.token0Price).toFixed(2) : '-'
  const usdcPerKcs = usdcPair ? parseFloat(usdcPair.token0Price).toFixed(2) : '-'
  const usdtPerKcs = usdtPair ? parseFloat(usdtPair.token1Price).toFixed(2) : '-'

  return (
    <PriceCard>
      <AutoColumn gap="10px">
        <RowFixed>
          <TYPE.main>DAI/KCS: {formattedNum(daiPerKcs, true)}</TYPE.main>
          <TYPE.light style={{ marginLeft: '10px' }}>
            {daiPair && totalLiquidity ? formatPercent(daiPair.trackedReserveUSD / totalLiquidity) : '-'}
          </TYPE.light>
        </RowFixed>
        <RowFixed>
          <TYPE.main>USDC/KCS: {formattedNum(usdcPerKcs, true)}</TYPE.main>
          <TYPE.light style={{ marginLeft: '10px' }}>
            {usdcPair && totalLiquidity ? formatPercent(usdcPair.trackedReserveUSD / totalLiquidity) : '-'}
          </TYPE.light>
        </RowFixed>
        <RowFixed>
          <TYPE.main>USDT/KCS: {formattedNum(usdtPerKcs, true)}</TYPE.main>
          <TYPE.light style={{ marginLeft: '10px' }}>
