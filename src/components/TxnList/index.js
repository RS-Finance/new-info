...
function TxnList({ transactions, symbol0Override, symbol1Override, color }) {
  ...
  const ListItem = ({ item }) => {
    if (item.token0Symbol === 'WKCS') {
      item.token0Symbol = 'KCS'
    }

    if (item.token1Symbol === 'WKCS') {
      item.token1Symbol = 'KCS'
    }
    ...
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area="value">
          {currency === 'KCS' ? 'Ξ ' + formattedNum(item.valueKCS) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={color} external href={'https://explorer.kcc.io/en/address/' + item.account}>
